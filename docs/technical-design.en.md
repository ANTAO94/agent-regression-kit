...
Based on v4.36.1 source. Package version 4.36.1, PUBLIC_API_VERSION=4 and Trace/Session/Contract/Report schema=0.1 are independent compatibility boundaries; benchmark manifest/decision/score/performance/study/readiness use their own schema 0.1.
...
`path_rules.ignore_argument_paths` is separate from
`state_equivalence.ignore_argument_paths`. The former controls argument
matching for the current path rules and is useful for unmodeled transport
noise such as `request_id`; it removes a field only when the baseline rule does
not declare it. If the baseline declares the field, its candidate value must
still match. The latter only groups already-declared outcome intents and does
not replace path argument assertions.
...
If a LangGraph tool runs inside an ordinary Python node, the completed
`messages` state may not contain its lifecycle. Collect
`graph.astream_events(..., version="v2")` and call
...
v4.19 also adds a telecom adapter. With `include_user_tools=True`, simulator
user-owned calls remain in the Trace with a `requestor` field; the Contract
checks assistant-owned writes only, while bounded parsers inspect service
status, mobile data, speed, MMS, data refueling and overdue-bill evidence. The
published telecom result is 147/217/0/0 over 364 eligible scenarios; the
prospective o4-mini result is 136/216/9/3. See the [v4.19 acceptance
record](v4.19-acceptance.md) and [telecom reproduction](../examples/tau2-telecom/README.md).

v4.20 adds a task-disjoint holdout proxy on top of that boundary.
`split_tau2_payload_by_task` reads task IDs only, assigns them to mutually
exclusive calibration and holdout buckets with SHA-256, verifies the task-set
digests, and then evaluates the selected telecom partition with the same
Contract. The published 28-task holdout contains 100 eligible scenarios and
produces 47/53/0/0; the prospective o4-mini partition produces 50/46/4/0,
with a 7.41% false-alarm rate. Both partitions still come from the same
published task family, so this is a reproducible task-level holdout proxy, not
...
v4.22 adds `examples/agentdojo_matrix_validation.py` on the same boundary. It
reads a pinned AgentDojo matrix manifest, verifies immutable revision,
per-result SHA-256, suite/task/attack identity and a reviewed Contract for each
case, writes redacted per-case Trace/report artifacts and emits an aggregate
gate. The matrix covers workspace, banking, Slack and travel, including direct
...
v4.24 adds `contract_sha256` for each matrix case using sorted, compact JSON,
and requires `contract_provenance.frozen_before_oracle=true` in a pre-registered
manifest. The validator checks the digest before evaluating the run and emits
per-case `contract_provenance` plus aggregate
`all_contract_provenance_bound` checks. A missing or modified Contract fails
closed. This proves rule provenance, not Contract completeness or semantic
correctness; see the [v4.24 acceptance record](v4.24-acceptance.md).

v4.25 adds `agentdojo_repeatability_validation.py`, which repeats the decision
pipeline for a pinned manifest and compares aggregate-report, case-report and
Trace SHA-256 values. CI runs it three times. This proves deterministic artifact
generation over fixed exported inputs, not online model sampling variance or
universal reliability; see the [v4.25 acceptance record](v4.25-acceptance.md).

v4.26 adds a separate `ignore_previous` attack-family matrix across four
AgentDojo suites. Each case keeps a manually reviewed Contract, a
pre-registered digest, external-oracle separation and an explicit expected
...
v4.29 adds a recorded sampling-study boundary: `study` reads redacted Traces
and a manifest supplied by the integrator, validates provider/model identity,
input and tool-schema SHA-256 values, sampling parameters, run-ID/Trace
binding, Contract/policy and path containment. It makes external Agent runs
auditable without owning provider execution or receiving API keys; finite
samples are not presented as population reliability. See the [v4.29
acceptance record](v4.29-acceptance.md).

v4.30 adds file-level evidence integrity to that boundary. A study can require
SHA-256 digests for the baseline, every run Trace and the normalized comparison
policy; the report exposes `evidence_integrity` and a tampered Trace is
rejected with CLI status 2. This proves file identity, not hidden-input
correctness or sample representativeness. See the [v4.30 acceptance
record](v4.30-acceptance.md).

v4.31 adds an `evidence` source inventory on top of those hashes. Integrators
can declare input, tool-schema, adapter, dataset, environment or provider-output
descriptors; the evaluator validates containment, uniqueness, required roles and
digests, then emits a content-free `evidence_index`. This improves provenance
reviewability but does not prove semantic correctness or source completeness.
See the [v4.31 acceptance record](v4.31-acceptance.md).

v4.32 adds `evidence_bindings` on top of the evidence index. Integrators can
bind a controlled descriptor field to `provenance.input_sha256`,
`provenance.tool_schema_sha256` or `provenance.adapter`. The evaluator checks
the file digest first, then the declared field value; even a hash-valid but
wrong source binding fails with CLI status 2. Reports contain binding IDs and
targets, not descriptor contents. See the [v4.32 acceptance record](v4.32-acceptance.md).

v4.33 extends the same boundary to run identity: `provenance.provider` and
`provenance.model` bind to separate `provider_output` descriptors, while
`provenance.dataset_revision` binds to a `dataset` descriptor. The new targets
are optional unless required by policy, so v4.32 manifests remain compatible.
See the [v4.33 acceptance record](v4.33-acceptance.md).

v4.34 adds a report-handoff sidecar: `study --checksum-out` computes a standard
SHA-256 over the final JSON, Markdown or JUnit bytes and requires explicit
`--out`. This lets CI or an audit system detect replacement after rendering,
without changing manifest compatibility. See the [v4.34 acceptance record](v4.34-acceptance.md).

v4.35 adds `readiness`: a final-v4 manifest re-reads structured benchmark and
performance fields, verifies referenced SHA-256 values and checks the final
sample/metric/performance gates. A real first-user study and a genuinely unseen
task domain are `external` checks; without independent evidence they remain
...
Current release: [v4.36.1](https://github.com/ANTAO94/agent-regression-kit/releases/tag/v4.36.1).
...
[Core CI](https://github.com/ANTAO94/agent-regression-kit/actions) · [Framework checks](https://github.com/ANTAO94/agent-regression-kit/actions) · [Performance CI](https://github.com/ANTAO94/agent-regression-kit/actions/workflows/performance.yml) · [Refund business case](../examples/refund-business-case/README.md) · [Path variation](../examples/path-variation/README.md) · [DeepSeek live check](deepseek-live.md) · [Independent LangGraph pilot](p1-langgraph-agent-stack-validation.md) · [Independent tau2 validation](tau2-independent-validation.md) · [Independent consumer pilot](consumer-pilot.md) · [Recorded sampling study](../examples/sampling-study/README.md) · [Readiness audit](readiness-audit.md) · [Performance baseline](performance.md) · [State-equivalence guide](state-equivalence.md) · [Telecom reproduction](../examples/tau2-telecom/README.md) · [v4.36.1 acceptance](v4.36.1-acceptance.md) · [v4.36 acceptance](v4.36-acceptance.md) · [AgentDojo attack-family](v4.26-acceptance.md) · [AgentDojo repeatability](v4.25-acceptance.md) · [AgentDojo Contract pre-registration](v4.24-acceptance.md) · [AgentDojo cross-model attack matrix](v4.23-acceptance.md) · [AgentDojo matrix](v4.22-acceptance.md) · [AgentDojo single-sample intake](v4.21-acceptance.md) · [Release integrity](supply-chain.md) · [v4.31 acceptance](v4.31-acceptance.md) · [v4.32 acceptance](v4.32-acceptance.md) · [v4.33 acceptance](v4.33-acceptance.md) · [v4.34 acceptance](v4.34-acceptance.md) · [Release](https://github.com/ANTAO94/agent-regression-kit/releases/tag/v4.36.1) · [v4.29 acceptance](v4.29-acceptance.md) · [v4.28 acceptance](v4.28-acceptance.md) · [v4.27 acceptance](v4.27-acceptance.md) · [v4.26 acceptance](v4.26-acceptance.md) · [v4.25 acceptance](v4.25-acceptance.md) · [v4.24 acceptance](v4.24-acceptance.md) · [v4.23 acceptance](v4.23-acceptance.md) · [v4.22 acceptance](v4.22-acceptance.md) · [v4.21 acceptance](v4.21-acceptance.md) · [v4.20 acceptance](v4.20-acceptance.md) · [v4.19 acceptance](v4.19-acceptance.md) · [v4.18 acceptance](v4.18-acceptance.md) · [v4.17 acceptance](v4.17-acceptance.md) · [v4.16 acceptance](v4.16-acceptance.md) · [v4.15 acceptance](v4.15-acceptance.md) · [v4.14 acceptance](v4.14-acceptance.md) · [v4.13 acceptance](v4.13-acceptance.md) · [v4.12 acceptance](v4.12-acceptance.md) · [v4.11 acceptance](v4.11-acceptance.md)
