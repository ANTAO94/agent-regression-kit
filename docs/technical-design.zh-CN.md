...
| 批量评测 | batch.py、batch_record.py、stability.py、coverage.py | 场景集/多次运行 → 聚合报告 | 有界并发、按用例 Contract、失败归集、阈值 |
