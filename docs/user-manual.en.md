...
For v4.1.0. Commands assume Bash/Zsh on macOS/Linux, run from the repository root unless stated otherwise. Python ≥3.9 is required; CI tests 3.9, 3.11 and 3.13. Installation needs network access; default examples need no model credentials.
...
A runnable policy is provided in examples/quickstart/compare.config.json in v4.1.0. After recording the candidate:
...
jobs:
  regression:
    ...
    steps:
      ...
      - name: Install
        id: install
        run: python -m pip install "git+https://github.com/ANTAO94/agent-regression-kit.git@v4.1.0"
      ...
      - name: Index reports
        ...
        uses: ANTAO94/agent-regression-kit/.github/actions/agent-report-index@v4.1.0
        with:
...
Trace events. See the [v4.1 acceptance contract](v4.1-acceptance.md) for the
