...
- [x] manifest 可要求 baseline、每个 run Trace 和规范化 comparison policy 的 SHA-256。
...
- [x] `evidence_bindings` 把 input、tool schema、adapter descriptor 的受控字段绑定到对应 provenance 值。
...
- [x] `study --checksum-out` 对最终写出的 JSON、Markdown 或 JUnit 报告生成标准 SHA-256 sidecar。
