...
v3.5 核心已经可以作为开发团队的 Agent 回归测试工具使用；后续版本重点是
让工具重放、路径约束、真实框架事件接入、本地审核和兼容发布形成闭环。
...
- [ ] 将已审核 Trace 转换为严格顺序的工具 cassette。
- [ ] 允许 Agent 代码在不触碰真实工具的情况下重跑并验证参数。
- [ ] 对多余调用、漏调用、工具名/参数不匹配输出结构化失败原因。
- [ ] 保留 `replay` 的“只读查看证据”语义，避免命令含义混淆。

### v3.7：合法路径与结果关联

- [ ] 让路径规则可以约束工具结果和错误状态，而不只是工具名称。
- [ ] 以 call_id 关联结果，降低并发/事件到达顺序造成的误报。
- [ ] 增加分支覆盖和替代路径的最小充分断言示例。
...
- [ ] Viewer 支持从一个工作区入口查看策略、Trace、差异和报告完整性。
- [ ] 增加只读 review 流程；baseline 接受必须由命令/API 显式触发。
- [ ] 为报告、策略和 Trace 增加可追溯的 manifest 与脱敏摘要。

### v4.0：稳定兼容契约

- [ ] 发布稳定的 v4 public API、迁移检查和弃用策略。
- [ ] 固化 Trace/Contract/Report 的兼容边界与安装级验收。
- [ ] 完成全量离线夹具、框架示例、CI、wheel 安装和安全门禁。
- [ ] 在真实团队使用后再评估可选服务化层：
  - 报告持久化和检索；
  - 项目、用户、权限和审计；
