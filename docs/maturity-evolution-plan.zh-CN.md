...
> 后续工作排序已由[真实接入与回归价值迭代方案](product-iteration-plan.zh-CN.md)接替。本文保留历史设计与验收记录；未完成的外部验证仍未完成。
...
### 7.15 v4.27：独立 Claude 模型族矩阵（已落地）

v4.27 把“更多模型族”从待办推进为一个固定的 AgentDojo manifest：四条
`claude-3-5-sonnet-20241022` pipeline 的 `important_instructions` 样本覆盖四个 suite，继续
使用相同的 Contract provenance、外部 oracle 隔离和三次重复性 gate。

- [x] 增加 `matrix-v4.27.json`，四条样本来自固定 revision，Contract 使用 canonical JSON SHA-256
  预注册，并显式声明 expected outcome。
- [x] workspace、banking、travel 通过，slack 因缺少必要查询按预期阻断，实测 4/4 matrix gate
  通过、1 条实际 Contract 阻断。
- [x] 增加 `agentdojo-claude-model` CI job，执行矩阵验证、三次重复性检查并上传完整 artifact。
- [x] 本地测试达到 267 项，双语 README、手册、技术设计和 v4.27 验收文档同步更新。
- [x] v4.27 release wheel、核心 267 项 CI、独立来源 CI 以及独立 consumer wheel 接入均有可点击的远程证据。
...
### 7.18 v4.30：study 证据文件完整性（已落地）

v4.30 把 v4.29 的记录式 study 从“可审计的声明”推进到“可检测的文件证据边界”：manifest
可以要求 baseline、每个 run Trace 和规范化 comparison policy 的 SHA-256。评估器先校验完整性，
再进入既有的 Contract、Stability 和 Wilson 区间计算。

- [x] 新增 `sha256_file` 公共 helper，支持对磁盘证据文件计算 SHA-256。
- [x] `integrity.require_trace_hashes=true` 时要求 baseline hash 和每个 run 的 `sha256`，并拒绝
  缺失或不匹配的摘要。
- [x] 支持 `comparison_policy_sha256`，绑定规范化运行时策略，避免 study bundle 内策略漂移。
- [x] 报告新增 `evidence_integrity`，输出观察到的 baseline、policy 和逐次 run 摘要，不嵌入 Trace 原文。
- [x] 核心 CI 增加篡改负向用例：修改临时目录内的 run Trace 后，CLI 必须返回状态 2。
- [x] v4.30 本地测试达到 279 项；v4.29 未带 integrity 的 manifest 继续兼容。
...
### 7.21 v4.33：study 运行身份绑定（已落地）

v4.33 处理 v4.32 之后仍存在的归因歧义：即使 input、tool schema 和 adapter 的来源绑定正确，
一个 study 仍可能把结果归因到错误的 provider、model 或 dataset revision。新版本复用同一个
`evidence_bindings` 机制，把这些身份拆成显式角色和受控字段。

- [x] 支持 `provenance.provider`、`provenance.model` 和 `provenance.dataset_revision`，分别要求 `provider_output`、`provider_output` 和 `dataset` role。
- [x] 新增目标可由 `required_evidence_bindings` 选择性启用；未启用新目标的 v4.32 manifest 继续兼容。
- [x] 示例 study、CLI 报告、核心 CI 负向场景和独立 consumer 均覆盖 provider/model/dataset 身份错配。
