...
Agent Regression Kit v4.34 deliberately stays small. The local Viewer
...
- The v4.23 AgentDojo matrix adds four gpt-4o direct controls and four
  gpt-4o-mini `important_instructions` attack trajectories. It proves that an
...
- The v4.26 attack-family matrix adds four `ignore_previous` cases across four
...
- v4.31 can add an evidence index for input, tool schema, adapter, dataset,
  environment or provider-output descriptors. The index makes declared source
  coverage reviewable, but it is not a signature system, semantic oracle or
  proof that the selected sources are complete.
- v4.32 can bind the controlled `input_sha256`, `tool_schema_sha256` or
  `adapter` field in an indexed JSON descriptor to the matching provenance
  value. This catches an unchanged-but-wrong source binding, but it still does
  not prove that the provenance declaration or descriptor is truthful.
- v4.33 can additionally bind provider, model and dataset-revision identity to
  explicitly indexed descriptors. This catches wrong run attribution, but it
  still does not prove that an external provider, model or dataset declaration
  is truthful, representative or complete.
- v4.34 can write a SHA-256 sidecar over the rendered study report. This catches
  replacement during artifact upload or review handoff, but it does not prove
  the report inputs, Contract or conclusions are correct.
