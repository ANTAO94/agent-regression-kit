...
core checkout. After the v4.25 release, that consumer was upgraded and
re-verified against the immutable v4.25 wheel:

[ANTAO94/agent-regression-pilot](https://github.com/ANTAO94/agent-regression-pilot)

v4.15 增加了一个与核心仓库分离的端到端消费项目；v4.25 发布后，消费项目又升级到
v4.25 wheel 并重新验收：
...
```text
https://github.com/ANTAO94/agent-regression-kit/releases/download/v4.25.0/agent_regression_kit-4.25.0-py3-none-any.whl
sha256: 650f502601a2af406ff9aa00120caf74642ba481da1227fc249f127d2046ff27
```
...
The consumer workflow ran on the v4.25 upgrade commit `36d3852` and passed
(GitHub Actions run [`35537976651`](https://github.com/ANTAO94/agent-regression-pilot/actions/runs/35537976651)):
...
`d438954` (metadata recorded in follow-up commit `cbcaa32`); the current v4.24
post-release verification uses the v4.24.0 wheel at `be445f1` (metadata recorded
in follow-up commit `48f34ab`); the current v4.25 post-release verification
uses the v4.25.0 wheel at `36d3852` (metadata recorded in follow-up commit
`b32d6bc`).

消费仓库的 CI（run `35537976651`）已验证正常场景返回 0，三类注入均返回 1。报告会保留在
