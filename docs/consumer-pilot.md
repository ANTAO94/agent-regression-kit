...
core checkout. After the v4.32 release, that consumer was upgraded and
re-verified against the immutable v4.32 wheel:

[ANTAO94/agent-regression-pilot](https://github.com/ANTAO94/agent-regression-pilot)

v4.15 增加了一个与核心仓库分离的端到端消费项目；v4.32 发布后，消费项目又升级到
v4.32 wheel 并重新验收：
...
```text
https://github.com/ANTAO94/agent-regression-kit/releases/download/v4.32.0/agent_regression_kit-4.32.0-py3-none-any.whl
...
The consumer workflow ran on the v4.32 upgrade commit `ec4ae11` and passed
(GitHub Actions run [`35546793151`](https://github.com/ANTAO94/agent-regression-pilot/actions/runs/35546793151)):
...
The v4.32 consumer imports the public `sha256_file` helper from the released
wheel, hashes its reviewed baseline and asserts `__version__ == 4.32.0` before
exercising the Agent boundary. It also builds an independent sampling-study
bundle, verifies the required `input`, `tool_schema` and `adapter` evidence
roles, and binds their controlled fields to matching provenance values with the
published `study` CLI. This confirms that the v4.32 semantic-binding boundary
is available through the published artifact, not only from the core checkout.

v4.32 消费项目从发布 wheel 导入公开的 `sha256_file`，对审核过的 baseline 计算哈希，并在
运行 Agent 边界前断言 `__version__ == 4.32.0`；同时由独立消费方创建 sampling-study bundle，
通过发布的 `study` CLI 校验 `input`、`tool_schema`、`adapter` 三类必需 evidence role，并把
描述字段绑定到对应 provenance 值。这证明 v4.32 语义绑定边界确实随发布包可用，而不是只在核心仓库源码路径中可用。
...
commit `47a6300`). The current v4.32 post-release verification uses the v4.32.0
...
消费仓库的 v4.32 CI（run `35546793151`）已验证正常场景返回 0，三类注入均返回 1，验证公开
`sha256_file` API，并用消费方独立生成的 study bundle 验证三个 evidence role 和三条 provenance
binding。报告会保留在 workflow artifact 中，baseline 由人工审核后提交，CI 不会自动覆盖 baseline。
v4.31 的消费验收仍保留在 run `35545604117`，v4.30 仍保留在 run `35544440919`。
