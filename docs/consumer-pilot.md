...
core checkout. After the v4.19 release, that consumer was upgraded and
re-verified against the immutable v4.19 wheel:

[ANTAO94/agent-regression-pilot](https://github.com/ANTAO94/agent-regression-pilot)

v4.15 增加了一个与核心仓库分离的端到端消费项目；v4.19 发布后，消费项目又升级到
v4.19 wheel 并重新验收：
...
```text
https://github.com/ANTAO94/agent-regression-kit/releases/download/v4.19.0/agent_regression_kit-4.19.0-py3-none-any.whl
sha256: c4051e9f77ed370570f55a7408425c6a9177e1988319499c5a00c8b9bfdbba8e
```
...
The consumer workflow ran on the v4.19 upgrade commit `fa85679` and passed
(GitHub Actions run [`35530368544`](https://github.com/ANTAO94/agent-regression-pilot/actions/runs/35530368544)):
...
`15cea6c`; the current v4.19 post-release verification uses the v4.19.0 wheel
at `fa85679`.

消费仓库的 CI（run `35530368544`）已验证正常场景返回 0，三类注入均返回 1。报告会保留在
