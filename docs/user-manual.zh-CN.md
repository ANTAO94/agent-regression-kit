...
适用：v4.2.0。以下命令面向 macOS/Linux Bash 或 Zsh，默认在仓库根目录执行。核心包要求 Python ≥ 3.9；远端矩阵覆盖 3.9、3.11、3.13。首次安装需要联网，默认离线示例无需模型 API Key。
...
可运行的比较策略示例位于 v4.2.0 的 examples/quickstart/compare.config.json。已有上节 candidate 后执行：
...
清单见 [v4.2 验收说明](v4.2-acceptance.md)。
