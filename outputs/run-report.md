# Agent Regression Kit v1.8 verification
...
- Unit and integration tests: 58 passed, 0 failed.
- Wheel build: `agent_regression_kit-1.8.0-py3-none-any.whl` succeeded.
...
The bundled fixture is a deliberately narrow 2025-11-25 test server, not a full conformance implementation. The client supports the documented v1.8 stdio and Streamable HTTP subset, but OAuth negotiation, every optional MCP capability, the handshake-free 2026-07-28 protocol, and broad framework compatibility remain outside the current release line. Default tests stay offline. Claims-only mode is explicit structural comparison, not semantic judging or an LLM-based evaluator.
