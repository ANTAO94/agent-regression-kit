# Agent Regression Kit v2.3 verification
...
- Unit and integration tests: 70 passed, 0 failed.
- Wheel build: `agent_regression_kit-2.3.0-py3-none-any.whl` succeeded.
...
The bundled fixture is a deliberately narrow 2025-11-25 test server, not a full conformance implementation. The client supports the documented v2.3 stdio and Streamable HTTP subset, but OAuth negotiation, every optional MCP capability, the handshake-free 2026-07-28 protocol, and broad framework compatibility remain outside the current release line. Default tests stay offline. Contract assertions, state snapshots, path coverage, and normalizers are deterministic; claims-only mode is explicit structural comparison, not semantic judging or an LLM-based evaluator.
