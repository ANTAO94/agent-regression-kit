...
Python ≥3.9 · 当前版本 v4.30.0 · 核心无必需第三方运行时依赖。
...
应看到 `agent-regression 4.30.0`。后续命令均在仓库根目录执行，并保持虚拟环境已激活。
...
当前适合本地开发与团队 CI 试点。v4.30 发布记录 **279 项测试通过**，并验证构建、干净环境安装、
首用模板、性能 smoke、独立消费仓库升级、多个任务域的公开/前瞻评测，以及记录式 study 证据的
baseline/run/policy 完整性校验。
...
当前版本的采样证据：[v4.30 验收记录](docs/v4.30-acceptance.md)。
