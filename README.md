...
当前版本：[v4.5.0](https://github.com/ANTAO94/agent-regression-kit/releases/tag/v4.5.0)。Python ≥3.9，核心无必需第三方运行时依赖，MIT 开源。
...
这些结果证明当前框架能够统一接收不同 Agent 运行时的证据、发现真实参数回归，并把在线模型行为放进 CI；它们还不能证明所有模型、所有多 Agent 协作或长期生产负载都已覆盖。完整边界见 [v4.5 验收说明](docs/v4.5-acceptance.md)。
...
| 了解 v4.5 验收与升级 | [v4.5 验收](docs/v4.5-acceptance.md) · [升级说明](UPGRADING.md) |
...
| 下载 | [wheel 与源码包](https://github.com/ANTAO94/agent-regression-kit/releases/tag/v4.5.0) |
...
Release: [v4.5.0](https://github.com/ANTAO94/agent-regression-kit/releases/tag/v4.5.0). Python ≥3.9, no required third-party core runtime dependencies, MIT license.
...
production workload. See the [v4.5 acceptance contract](docs/v4.5-acceptance.md).
...
| Read the v4.5 acceptance and upgrade contract | [v4.5 acceptance](docs/v4.5-acceptance.md) · [Upgrade guide](UPGRADING.md) |
...
Recorded v4.5.0 evidence is maintained by the main regression, framework compatibility, live-provider and release workflows; each release also includes local full-test, wheel-build, compatibility, migration and clean-install checks.
