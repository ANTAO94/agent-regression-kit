...
当前版本：[v4.11.0](https://github.com/ANTAO94/agent-regression-kit/releases/tag/v4.11.0)。Python ≥3.9，核心无必需第三方运行时依赖，MIT 开源。
...
| τ²-bench 零售数据 | 独立项目发布的 456 条 `gpt-4.1-mini` 工具 Agent 轨迹；reward 在 Contract 判断后才读取 | 420 条写场景：正确放行 253、正确阻断 153、误报 14、漏报 0；准确率 96.67% | [独立验证工作流](https://github.com/ANTAO94/agent-regression-kit/actions/workflows/tau2-independent-validation.yml) · [方法与结果](docs/tau2-independent-validation.md) |
...
τ²-bench 的 14 个误报被保留在报告中：它们说明参考写操作并不总是唯一正确路径，最终数据库状态等价仍需要更强的状态契约。这份结果证明框架能够统一接收不同运行时证据、发现真实失败并量化自身误报；它不代表上游项目采用或认可本框架。完整边界见 [v4.11 验收说明](docs/v4.11-acceptance.md)。
...
| 了解 v4.11 验收与升级 | [v4.11 验收](docs/v4.11-acceptance.md) · [v4.10 验收](docs/v4.10-acceptance.md) · [升级说明](UPGRADING.md) |
...
| 独立项目实测 | [τ²-bench 公开零售轨迹](docs/tau2-independent-validation.md) · 420 条写场景 · 153 个失败全部阻断 · 14 个误报公开保留 |
| 下载 | [wheel 与源码包](https://github.com/ANTAO94/agent-regression-kit/releases/tag/v4.11.0) |
...
Release: [v4.11.0](https://github.com/ANTAO94/agent-regression-kit/releases/tag/v4.11.0). Python ≥3.9, no required third-party core runtime dependencies, MIT license.
...
| τ²-bench retail | 456 published `gpt-4.1-mini` tool-Agent trajectories with reward labels hidden until after each Contract decision | 420 write scenarios: 253 true passes, 153 true blocks, 14 false alarms and 0 missed failures; 96.67% accuracy | [Independent workflow](https://github.com/ANTAO94/agent-regression-kit/actions/workflows/tau2-independent-validation.yml) · [Method and evidence](docs/tau2-independent-validation.md) |
...
or endorsement. See the [v4.11 acceptance contract](docs/v4.11-acceptance.md).
...
| Read the v4.11 acceptance and upgrade contract | [v4.11 acceptance](docs/v4.11-acceptance.md) · [v4.10 acceptance](docs/v4.10-acceptance.md) · [Upgrade guide](UPGRADING.md) |
...
Recorded v4.11.0 evidence is maintained by the main regression, framework compatibility, path-variation, refund, τ²-bench, live-provider and release workflows; each release also includes local full-test, wheel-build, compatibility, migration and clean-install checks.
