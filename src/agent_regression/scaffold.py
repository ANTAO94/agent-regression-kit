...
This folder is generated by `agent-regression init`. It contains a reviewed
baseline, a candidate trace, a strict Contract and a GitHub Actions workflow.
...
- `baselines/`: reviewed evidence; update only after a deliberate behavior review.
...
The generated starter baseline is `my-agent.trace.json`. Keep its business
meaning stable even if run-scoped IDs or transport metadata change.
""",
