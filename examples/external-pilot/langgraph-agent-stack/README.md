...
git checkout a8a2dac566d46c48619ba94c69dfffb1b370520d8
uv sync
```
...
cp "$KIT/examples/external-pilot/langgraph-agent-stack/baseline.trace.json" \
   "$KIT/examples/external-pilot/langgraph-agent-stack/candidate.trace.json"
...
预期：正常候选退出码为 `0`，报告为 `passed: true`，并且 Trace 至少包含 3 个
`web_search` 工具调用。`claims` 来自候选项目实际输出的 `summary`、`findings`、
`sources` 和 `confidence`，不是脚本预填的“永远通过”答案。
