"""Validate Agent Regression Kit against pinned tau2-bench airline results."""
...
from agent_regression import evaluate_tau2_airline_results, trace_from_tau2_simulation


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_SOURCE = ROOT / "examples" / "tau2-airline" / "source.json"
...
def run(args: argparse.Namespace) -> int:
    ...
    report = evaluate_tau2_airline_results(
        payload,
        source=source,
...
def main() -> int:
    ...
    try:
        ...
        print(f"tau2 airline validation input error: {exc}", file=sys.stderr)
        return 2
